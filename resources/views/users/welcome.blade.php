                ...
                @if(!empty($hotels))
                    @foreach($hotels as $hotel)
                        ...
                        @foreach($hotel->room as $room)
                           ...
                           $price = $room->calendars->sum('room_price');
                                ...
                                <div class="card h-100">
                                    ...
                                    @if(!empty($hotel))
                                        ...
                                        @foreach(json_decode($hotel->hotel_photos) as $image)
                                            <img src="{{URL::to('/hotels/'.$image)}}" class="card-img-top" alt="...">
                                        @endforeach
                                    @endif
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row" style="align-items: flex-end;height: 100%">
                                            ...
                                            <div>
                                                ...
                                                @endphp
                                                <h1>{{lang() == 'ar' ? $hotel->name_ar : $hotel->name_en}}</h1>.
                                                <h4 class="card-title">{{ lang() == 'ar' ? $hotel->location_ar : $hotel->location_en }} </h4>.
                                                <br>

                                                <h6>{{$room->room_type}} </h6>
                                                <span>{{$room->adults_max}} {{__('welcome.adult')}} / {{$room->child_max }} {{__('welcome.child')}}</span><br><br>
                                                <span>{{__('welcome.stay')}}  {{$days}} {{__('welcome.night')}}  </span><br>
                                                <span>{{__('welcome.total')}}  {{number_format($price,2)}}  {{ lang() == 'ar' ? $hotel->currency_ar :  $hotel->currency_en}} </span>
